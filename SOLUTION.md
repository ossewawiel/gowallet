...
> subquery that reuses the *exact* Σ(earn)−Σ(spend) formula as `BalanceForAccount`, so a list row
...
registry. **INV-1–23 are now all proven (green under `-race`)** — accounts, earn, spend, auth,
audit, batch, login, **and listings** (INV-18/19/20 landed with S7).
...
- **`int64` balance overflow** is flagged as a known edge (noted during the S3 build) to be handled
  where it belongs.
- **The full core backlog (S0–S7) is now merged** — accounts, earn, spend, auth, audit, CSV batch,
  login, and listings. Remaining work is hardening/polish (see [`docs/SLICES.md`](docs/SLICES.md)).
...
the quality gate). Last synced after **S7 (listings) landed** on 2026-06-19: added `GET /accounts`
(admin) + `GET /accounts/{id}/transactions` (own/admin) over the existing tables (no migration),
INV-18/19/20 proven. **All core slices S0–S7 are now shipped.**_
