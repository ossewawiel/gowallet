...
**S0–S6 are built** — accounts, earn, spend, auth, audit, CSV batch ingestion, **and real
credential login**. Only **S7 (listings)** remains; it's fully designed in
[`docs/slices/`](docs/slices/) with GitHub issue #13 ready to build.
...
registry. **INV-1–17 and 21–23 are proven (green under `-race`)** — accounts, earn, spend, auth,
audit, batch, and login. Only **INV-18–20** remain (S7 listings).
...
- **`int64` balance overflow** is flagged as a known edge (noted during the S3 build) to be handled
  where it belongs.
- **Only listings (S7)** remains unbuilt — login, audit, and CSV batch ingestion are all merged.
...
the quality gate). Last synced after **S6 (login) landed** on 2026-06-19: `POST /login` replaces the
removed `POST /token`, INV-14..17 proven; only **S7 listings** remains._
