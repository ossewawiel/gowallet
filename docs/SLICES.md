...
The build plan: **6 vertical slices**, each one full REST cycle. Ordered for **3 parallel streams**,
...
| **S4** | Audit trail | audit table + writer | INV-11 | S0 | 🅲 |
...
- 🅱️ **Security:** `S3` starts as soon as **S0** is in — JWT is pure crypto + HTTP, no DB dep. Its
  ownership enforcement wires across S1/S2 **midstream**.
